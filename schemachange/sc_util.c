                    ...
                    struct schema_change_type *scopy)
{
    ...
    scopy->finalize = 0;
    if (strlen(db->tablename) + strlen("_history") + 1 > MAXTABLELEN) {
        sc_errf(s, "History table name too long\n");
        free_schema_change_type(scopy);
        return SC_CSC2_ERROR;
    }
    snprintf(scopy->tablename, sizeof(scopy->tablename), "%s_history", db->tablename);
    scopy->tablename[sizeof(scopy->tablename) - 1] = '\0';
